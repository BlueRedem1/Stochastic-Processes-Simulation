...
Markov<-function(pi,m,s,n){
  ...
  num_estados <- nrow(m) #o la longitud del vector s length(s)
  ...
  for(t in 2:n) {
    # Obtenemos el vector de probabilidades correspondiente a dicho estado en     el que caímos
    p  <- m[estados[t-1], ]
...
p4<-c(1/4,1/4,0,1/2)
m<-c(p1,p2,p3,p4)
m<-matrix(m,nrow=4,ncol=4,byrow = T)
Markov(pi,m,s,10)
```
